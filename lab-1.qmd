...
ggplot(
  ...
  geom_point() +
  labs(
    title = "Population density in comparison to land area of Northern Carolina Counties",
...
The scatterplot from question 8 and this scatterplot are identical in the relationship shown and distribution of the points. In both plots most counties have relatively low population densities, with fewer high density outliers.

They only are different in the unit used for quantifying the x axis. Question 8 uses miles and question 9 uses meters. The different unit used does not affect the positions of the data points and the relationship between the variables. As a result the visual representation of the data remains unchanged despite the different in miles and meters.
